...
var y = (s, i, t) => q(s, typeof i != "symbol" ? i + "" : i, t);
const c = {
  primary: "#00685d",
  accent: "#26a69a",
  text: "#0f1e1c",
...
class D extends HTMLElement {
  ...
  render() {
    ...
    const t = this.config(), e = this.mediaState(), a = (e == null ? void 0 : e.attributes) ?? {}, r = this.mergedTheme(), p = m(a.volume_level, 0, 1, 0.5), v = m(a.media_position, 0, Number(a.media_duration) || 1, 0), u = Math.max(0, Number(a.media_duration) || 0), x = o(a.media_title, e ? "Ничего не играет" : "Ожидание данных"), _ = o(a.media_artist || a.media_album_name, (e == null ? void 0 : e.state) === "unavailable" ? "Устройство недоступно" : "Яндекс.Станция"), w = o(a.entity_picture), E = m(t.artwork_size, 48, 220, 80), S = m(t.opacity, 0.1, 1, 0.72), M = m(t.blur, 0, 40, 18), h = t.content_align === "center" ? "center" : t.content_align === "right" ? "right" : "left", T = h === "center" ? "center" : h === "right" ? "flex-end" : "flex-start", P = t.controls_position === "left" ? "flex-start" : t.controls_position === "right" ? "flex-end" : "center", I = t.layout === "vertical" ? "ysp-vertical" : "", L = t.artwork_position === "right" ? "ysp-art-right" : t.artwork_position === "top" ? "ysp-art-top" : "", N = t.volume_on_hover === !0 ? "ysp-hover-volume" : "", Y = (e == null ? void 0 : e.state) === "unavailable" || !e, f = (e == null ? void 0 : e.state) === "playing", b = a.is_volume_muted === !0 || p === 0, $ = this.normalizedPresets(), k = this.normalizedCommands(), A = `
      ${F}
      :host { --ysp-primary:${l(o(r.primary), c.primary)}; --ysp-accent:${l(o(r.accent), c.accent)}; --ysp-text:${l(o(r.text), c.text)}; --ysp-secondary:${l(o(r.secondary), c.secondary)}; --ysp-surface:${l(o(r.surface), c.surface)}; --ysp-background:${l(o(r.background), c.background)}; --ysp-on-primary:${l(o(r.on_primary), c.on_primary)}; --ysp-border:${l(o(r.border), c.border)}; --ysp-shadow:${l(o(r.shadow), c.shadow)}; --ysp-art-size:${E}px; --ysp-blur:${M}px; --ysp-controls-position:${P}; --ysp-align:${h}; --ysp-align-items:${T}; }
      .ysp-card { opacity:${S}; }
    `, j = t.show_artwork === !1 ? "" : w ? `<img class="ysp-art" src="${n(w)}" alt="Обложка текущего трека">` : '<div class="ysp-art ysp-art-placeholder" aria-label="Нет обложки">♫</div>';
    this._root.innerHTML = `
      <style>${A}</style>
      <article class="ysp-card ${I} ${L} ${N}" aria-label="${n(t.name ?? "Яндекс.Станция")}" data-unavailable="${Y}">
        <div class="ysp-content">
          ${t.show_header !== !1 ? `<header class="ysp-header"><span class="ysp-name">${n(t.name ?? "Яндекс.Станция")}</span><button class="ysp-source" type="button" data-action="more-info" aria-label="Открыть информацию">${n(o(a.source, "Яндекс Музыка"))}⌄</button></header>` : ""}
          <section class="ysp-track">${j}<div class="ysp-track-info"><div class="ysp-title" title="${n(x)}">${n(x)}</div><div class="ysp-artist" title="${n(_)}">${n(_)}</div></div></section>
          ${t.show_progress !== !1 && u > 0 ? `<section class="ysp-progress" aria-label="Позиция воспроизведения"><span>${z(v)}</span><input type="range" min="0" max="${u}" step="1" value="${v}" data-action="seek" aria-label="Перемотка"><span>${z(u)}</span></section>` : ""}
          ...
          ${t.show_volume !== !1 ? `<section class="ysp-volume" aria-label="Громкость"><span class="ysp-volume-icon">${b ? "🔇" : "🔊"}</span><input class="ysp-range" type="range" min="0" max="1" step="0.01" value="${p}" data-action="volume" aria-label="Громкость"><button class="ysp-chip" type="button" data-action="mute" aria-label="${b ? "Включить звук" : "Выключить звук"}">${b ? "Звук" : "Mute"}</button></section>` : ""}
          ${t.show_presets !== !1 && $.length > 0 ? `<section class="ysp-section"><p class="ysp-label">Избранное</p><div class="ysp-preset-row">${$.map((d, g) => `<button class="ysp-preset" type="button" data-action="preset" data-index="${g}">${n(d.icon)} ${n(d.name)}</button>`).join("")}</div></section>` : ""}
          ${t.show_quick_commands !== !1 && k.length > 0 ? `<section class="ysp-section"><p class="ysp-label">Быстрые команды</p><div class="ysp-command-row">${k.map((d, g) => `<button class="ysp-command" type="button" data-action="command" data-index="${g}">${n(d.icon ?? "•")} ${n(d.name)}</button>`).join("")}</div></section>` : ""}
        </div>
      </article>
    `;
  }
  ...
  handleClick(t) {
    ...
    if (a === "more-info") {
      this.dispatchEvent(new CustomEvent("hass-more-info", { bubbles: !0, composed: !0, detail: { entityId: this.config().entity } }));
      return;
    }
    if (a === "play" && this.call("media_play"), a === "pause" && this.call("media_pause"), a === "previous" && this.call("media_previous_track"), a === "next" && this.call("media_next_track"), a === "mute" && this.call("volume_mute", { is_volume_muted: ((r = this.mediaState()) == null ? void 0 : r.attributes.is_volume_muted) !== !0 }), a === "command") {
...
customElements.get("yandex-station-player") || customElements.define("yandex-station-player", D);
