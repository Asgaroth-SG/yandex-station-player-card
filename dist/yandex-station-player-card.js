...
var U = (s, i, t) => i in s ? D(s, i, { enumerable: !0, configurable: !0, writable: !0, value: t }) : s[i] = t;
var y = (s, i, t) => U(s, typeof i != "symbol" ? i + "" : i, t);
const c = {
  primary: "#00685d",
  accent: "#26a69a",
  text: "#0f1e1c",
...
.ysp-chip-icon { width:16px; height:16px; --mdc-icon-size:16px; }
...
function d(s, i = "ysp-action-icon") {
  ...
  return `<ha-icon class="${p(i)}" icon="${p(e)}" aria-hidden="true"></ha-icon>`;
}
function B(s) {
  return d(s);
}
...
class R extends HTMLElement {
  ...
  render() {
    ...
    const t = this.config(), e = this.mediaState(), n = (e == null ? void 0 : e.attributes) ?? {}, a = this.mergedTheme(), r = u(n.volume_level, 0, 1, 0.5), v = u(n.media_position, 0, Number(n.media_duration) || 1, 0), f = Math.max(0, Number(n.media_duration) || 0), _ = o(n.media_title, e ? "Ничего не играет" : "Ожидание данных"), w = o(n.media_artist || n.media_album_name, (e == null ? void 0 : e.state) === "unavailable" ? "Устройство недоступно" : "Яндекс.Станция"), $ = o(n.entity_picture), E = u(t.artwork_size, 48, 220, 80), M = u(t.opacity, 0.1, 1, 0.72), I = u(t.blur, 0, 40, 18), g = t.content_align === "center" ? "center" : t.content_align === "right" ? "right" : "left", T = g === "center" ? "center" : g === "right" ? "flex-end" : "flex-start", P = t.controls_position === "left" ? "flex-start" : t.controls_position === "right" ? "flex-end" : "center", j = t.layout === "vertical" ? "ysp-vertical" : "", N = t.artwork_position === "right" ? "ysp-art-right" : t.artwork_position === "top" ? "ysp-art-top" : "", L = t.volume_on_hover === !0 ? "ysp-hover-volume" : "", Y = (e == null ? void 0 : e.state) === "unavailable" || !e, x = (e == null ? void 0 : e.state) === "playing", h = n.is_volume_muted === !0 || r === 0, A = h ? "mdi:volume-off" : "mdi:volume-high", H = h ? "mdi:volume-high" : "mdi:volume-mute", k = this.normalizedPresets(), z = this.normalizedCommands(), q = `
      ${W}
      :host { --ysp-primary:${l(o(a.primary), c.primary)}; --ysp-accent:${l(o(a.accent), c.accent)}; --ysp-text:${l(o(a.text), c.text)}; --ysp-secondary:${l(o(a.secondary), c.secondary)}; --ysp-surface:${l(o(a.surface), c.surface)}; --ysp-background:${l(o(a.background), c.background)}; --ysp-on-primary:${l(o(a.on_primary), c.on_primary)}; --ysp-border:${l(o(a.border), c.border)}; --ysp-shadow:${l(o(a.shadow), c.shadow)}; --ysp-art-size:${E}px; --ysp-blur:${I}px; --ysp-controls-position:${P}; --ysp-align:${g}; --ysp-align-items:${T}; }
      .ysp-card { opacity:${M}; }
    ...
    this._root.innerHTML = `
      <style>${q}</style>
      <article class="ysp-card ${j} ${N} ${L}" aria-label="${p(t.name ?? "Яндекс.Станция")}" data-unavailable="${Y}">
        <div class="ysp-content">
          ...
          ${t.show_progress !== !1 && f > 0 ? `<section class="ysp-progress" aria-label="Позиция воспроизведения"><span>${C(v)}</span><input type="range" min="0" max="${f}" step="1" value="${v}" data-action="seek" aria-label="Перемотка"><span>${C(f)}</span></section>` : ""}
          ...
          ${t.show_volume !== !1 ? `<section class="ysp-volume" aria-label="Громкость"><span class="ysp-volume-icon">${d(A, "ysp-volume-glyph")}</span><input class="ysp-range" type="range" min="0" max="1" step="0.01" value="${r}" data-action="volume" aria-label="Громкость"><button class="ysp-chip" type="button" data-action="mute" aria-label="${h ? "Включить звук" : "Выключить звук"}">${d(H, "ysp-chip-icon")}<span>${h ? "Unmute" : "Mute"}</span></button></section>` : ""}
          ${t.show_presets !== !1 && k.length > 0 ? `<section class="ysp-section"><p class="ysp-label">Избранное</p><div class="ysp-preset-row">${k.map((m, b) => `<button class="ysp-preset" type="button" data-action="preset" data-index="${b}">${B(m.icon)}<span>${p(m.name)}</span></button>`).join("")}</div></section>` : ""}
...
customElements.get("yandex-station-player") || customElements.define("yandex-station-player", R);
