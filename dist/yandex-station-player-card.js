...
.ysp-volume { gap:10px; width:100%; color:var(--ysp-secondary); }
...
.ysp-range { min-width:0; flex:1; height:6px; margin:0; accent-color:var(--ysp-primary); cursor:pointer; border-radius:999px; background:linear-gradient(to right, var(--ysp-primary) 0 var(--ysp-volume-pct), var(--ysp-surface) var(--ysp-volume-pct) 100%); }
.ysp-range::-webkit-slider-thumb { width:16px; height:16px; }
.ysp-range::-moz-range-thumb { width:16px; height:16px; }
.ysp-volume-value { min-width:36px; color:var(--ysp-secondary); font-size:12px; font-variant-numeric:tabular-nums; text-align:right; }
...
function d(s, a = "ysp-action-icon") {
...
class G extends HTMLElement {
  ...
  render() {
    ...
    const t = this.config(), e = this.mediaState(), i = (e == null ? void 0 : e.attributes) ?? {}, n = this.mergedTheme(), r = g(i.volume_level, 0, 1, 0.5), y = Math.max(0, Number(i.media_duration) || 0), f = V(i, e == null ? void 0 : e.state, y), S = y > 0 ? Math.round(f / y * 1e3) / 10 : 0, w = o(i.media_title, e ? "Ничего не играет" : "Ожидание данных"), $ = o(i.media_artist || i.media_album_name, (e == null ? void 0 : e.state) === "unavailable" ? "Устройство недоступно" : "Яндекс.Станция"), k = o(i.entity_picture), N = g(t.artwork_size, 48, 220, 80), E = g(t.opacity, 0.1, 1, 0.72), P = g(t.blur, 0, 40, 18), b = t.content_align === "center" ? "center" : t.content_align === "right" ? "right" : "left", T = b === "center" ? "center" : b === "right" ? "flex-end" : "flex-start", I = t.controls_position === "left" ? "flex-start" : t.controls_position === "right" ? "flex-end" : "center", j = t.layout === "vertical" ? "ysp-vertical" : "", A = t.artwork_position === "right" ? "ysp-art-right" : t.artwork_position === "top" ? "ysp-art-top" : "", L = "", H = (e == null ? void 0 : e.state) === "unavailable" || !e, v = (e == null ? void 0 : e.state) === "playing", x = i.is_volume_muted === !0 || r === 0, Y = x ? "mdi:volume-off" : "mdi:volume-high", q = x ? "mdi:volume-high" : "mdi:volume-mute", z = this.normalizedPresets(), M = this.normalizedCommands(), D = `
      ${R}
      :host { --ysp-primary:${l(o(n.primary), c.primary)}; --ysp-accent:${l(o(n.accent), c.accent)}; --ysp-text:${l(o(n.text), c.text)}; --ysp-secondary:${l(o(n.secondary), c.secondary)}; --ysp-surface:${l(o(n.surface), c.surface)}; --ysp-background:${l(o(n.background), c.background)}; --ysp-on-primary:${l(o(n.on_primary), c.on_primary)}; --ysp-border:${l(o(n.border), c.border)}; --ysp-shadow:${l(o(n.shadow), c.shadow)}; --ysp-art-size:${N}px; --ysp-blur:${P}px; --ysp-controls-position:${I}; --ysp-align:${b}; --ysp-align-items:${T}; }
      .ysp-card { opacity:${E}; }
    `, F = t.show_artwork === !1 ? "" : k ? `<img class="ysp-art" src="${p(k)}" alt="Обложка текущего трека">` : `<div class="ysp-art ysp-art-placeholder" aria-label="Нет обложки">${d("mdi:music-note", "ysp-art-icon")}</div>`;
    this._root.innerHTML = `
      <style>${D}</style>
      <article class="ysp-card ${j} ${A} ${L}" aria-label="${p(t.name ?? "Яндекс.Станция")}" data-unavailable="${H}">
        <div class="ysp-content">
          ...
          <section class="ysp-track">${F}<div class="ysp-track-info"><div class="ysp-title" title="${p(w)}">${p(w)}</div><div class="ysp-artist" title="${p($)}">${p($)}</div></div></section>
          ${t.show_progress !== !1 && y > 0 ? `<section class="ysp-progress" aria-label="Перемотка аудио"><span>${u(f)}</span><input class="ysp-seek-range ysp-seek" type="range" min="0" max="${y}" step="1" value="${f}" style="--ysp-seek-pct:${S}%" data-action="seek" aria-label="Перемотка аудио" aria-valuetext="${u(f)} из ${u(y)}"><span>${u(y)}</span></section>` : ""}
          ...
          ${t.show_volume !== !1 ? `<section class="ysp-volume" aria-label="Громкость"><span class="ysp-volume-icon">${d(Y, "ysp-volume-glyph")}</span><input class="ysp-range" type="range" min="0" max="1" step="0.01" value="${r}" data-action="volume" aria-label="Громкость" aria-valuetext="${Math.round(r * 100)} процентов" style="--ysp-volume-pct:${Math.round(r * 100)}%"><span class="ysp-volume-value">${Math.round(r * 100)}%</span><button class="ysp-chip ysp-mute-button" type="button" data-action="mute" aria-label="${x ? "Включить звук" : "Выключить звук"}">${d(q, "ysp-chip-icon")}<span>${x ? "Вкл." : "Выкл."}</span></button></section>` : ""}
          ${t.show_presets !== !1 && z.length > 0 ? `<section class="ysp-section"><p class="ysp-label">Избранное</p><div class="ysp-preset-row">${z.map((h, _) => `<button class="ysp-preset" type="button" data-action="preset" data-index="${_}">${U(h.icon)}<span>${p(h.name)}</span></button>`).join("")}</div></section>` : ""}
          ${t.show_quick_commands !== !1 && M.length > 0 ? `<section class="ysp-section"><p class="ysp-label">Быстрые команды</p><div class="ysp-command-row">${M.map((h, _) => `<button class="ysp-command" type="button" data-action="command" data-index="${_}">${d(h.icon ?? "mdi:gesture-tap-button")}<span>${p(h.name)}</span></button>`).join("")}</div></section>` : ""}
        </div>
      </article>
    `;
  }
  ...
  handleClick(t) {
    ...
    if (i === "play" && this.call("media_play"), i === "pause" && this.call("media_pause"), i === "previous" && this.call("media_previous_track"), i === "next" && this.call("media_next_track"), i === "mute" && this.call("volume_mute", { is_volume_muted: ((n = this.mediaState()) == null ? void 0 : n.attributes.is_volume_muted) !== !0 }), i === "command") {
  ...
  handleInput(t) {
    ...
    if (e) {
      if (e.dataset.action === "volume") {
        const n = Math.round(Number(e.value) * 100);
        this.call("volume_set", { volume_level: Number(e.value) }), e.setAttribute("aria-valuetext", `${n} процентов`), e.style.setProperty("--ysp-volume-pct", `${n}%`);
        const r = (i = e.parentElement) == null ? void 0 : i.querySelector(".ysp-volume-value");
        r && (r.textContent = `${n}%`);
      }
      e.dataset.action === "seek" && (this.call("media_seek", { seek_position: Number(e.value) }), e.setAttribute("aria-valuetext", `${u(Number(e.value))} из ${u(Number(e.max))}`), e.style.setProperty("--ysp-seek-pct", `${Number(e.max) > 0 ? Number(e.value) / Number(e.max) * 100 : 0}%`));
    }
  }
}
