...
Карточка поддерживает визуальный редактор Home Assistant: выбор `media_player`, название, иконка, позиции, layout, обложка, выравнивание трека, положение кнопок, размеры, прозрачность, blur и переключатели видимости. Пресеты, быстрые команды и тема редактируются через YAML-поля внутри визуального редактора.

После обновления ресурса очистите кэш браузера (`Ctrl+F5`). Если редактор не появился, проверьте, что Lovelace resource указывает на свежий `dist/yandex-station-player-card.js` и имеет `type: module`.
